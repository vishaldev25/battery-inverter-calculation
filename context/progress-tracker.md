...
- **Feature 08 (Backend API):** Created `backend/projects/router.py` — stateless `POST /api/projects/calculate` wiring the full 5-stage pipeline with per-stage hard-error early-exit.
...
- **Feature 11 (Catalog):** 
  ...
  - Created `backend/catalog/matcher.py` — `find_battery_candidates`, `get_battery_by_id`, `find_inverter_candidates`, `get_inverter_by_id`. Recommend-mode filters implement the sizing spec's hard gates (integer series-string pre-filter for batteries; continuous/surge VA, voltage window, waveform, grid-tie certification, and LVD coordination for inverters) as MongoDB filter queries — no formula duplication from `backend/calculation/`.
...
## Open Questions
- **Feature 08 field alignment:** `router.py`'s `CalculationRequest`/pipeline calls reference fields like `payload.parameters.system_dc_voltage` on `ProjectParameters` — not yet cross-checked against the real `backend/projects/models.py` (only `backend/catalog/models.py` has been shared/verified so far). Needs a check before Feature 12's first real end-to-end save test.
