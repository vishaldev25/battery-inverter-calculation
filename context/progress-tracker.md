...
## Current Phase
- Phase 3 - API & Routing. Feature 15 complete and fully verified (23/23 smoke test checks passing, including one route-ordering bug caught and fixed in-session).

## Current Goal
- Feature 16: `backend/report/` — PDF generation (ReportLab), reading `last_calculation_result` only, no recalculation, fixed light-mode styling per `ui-context.md`.
...
- **Feature 15 (CSV upload route wiring) — COMPLETE, FULLY VERIFIED:**
  - `backend/projects/router.py` extended with two routes: `GET /api/projects/csv-template` and `POST /api/projects/{project_id}/loads/csv-upload`.
  - Upload flow: extension check → size guard (5 MB) → empty-file check → `parse_csv_bytes()` (structural) → row-count guard (10,000) → `validate_rows()` (per-row, against real `LoadItem`) → all-or-nothing commit.
  - Strict all-or-nothing commit; append-only (not replace) to `project.loads`, per user decisions in the finalized spec.
  - No changes made to `backend/csv/parser.py`, `validator.py`, or `template.py` — used exactly as Feature 13 built them.
  - Code verified against real `backend/csv/parser.py`, `models.py`, and `template.py` sources (user-pasted): `CsvStructureError.message` confirmed always set; `CsvValidationResult` confirmed as a Pydantic `BaseModel` with matching field names; `template.py`'s blank `power_factor` example row confirmed to align with the earlier PF-resolution fix.
  - **Route-ordering bug found and fixed via smoke test (see Resolved Issues #14):** `GET /csv-template` was shadowed by `GET /{project_id}` because it was registered later in the file — FastAPI matches routes in registration order. Fixed by moving `download_csv_template_endpoint`'s registration to immediately before `GET /{project_id}`.
  - **Smoke-tested end-to-end via `_smoke_test_15.py`** (throwaway, FastAPI `TestClient` against the real app + real MongoDB, deleted post-verification): template byte-identical download, all-valid upload (append confirmed, not replace, PF correctly `None`), mixed-validity upload (zero DB changes confirmed, full per-row report, `template_url` present), malformed headers (400, not a raw 500), empty file (400), wrong extension (400), oversized file >5MB generated and confirmed rejected before parsing. **23/23 checks passing.**
  - **First run of the smoke test itself required its own fix**, unrelated to route code: `TestClient(app)` must be used as a context manager (`with TestClient(app) as client:`) for FastAPI's `lifespan` (`connect_to_mongo`/`close_mongo_connection`) to actually run — a bare `TestClient(app)` skips startup entirely, which was the cause of the first run's `RuntimeError: Database has not been initialized`.

## In Progress
- None — Feature 15 fully implemented, verified, and closed out.

## Next Up
- **Feature 16 (Report):** `backend/report/` — PDF generation (ReportLab), reading `last_calculation_result` only, no recalculation, fixed light-mode styling per `ui-context.md` (title block, executive summary, load schedule table, formulas for transparency, load analysis charts, safety notes — per `ui-context.md`'s PDF Export section). Spec not yet drafted this session.
...
- **[Carried, unresolved] PF-resolution fix (models.py/load.py) still needs its own dedicated smoke test** — Feature 15's smoke test incidentally covered one case (Test 2e: uploaded row's `power_factor` resolves to `None`, not `1.0`), but the full resolution logic (explicit override vs. category default vs. surge-calc usage) hasn't been exercised end-to-end through `calculate_load_profile()` directly. Lower priority now that Feature 15 confirms the model-level behavior is correct.
- **[Carried] `queries.py`'s regex-escape + secondary `_id` sort fixes still need a smoke-test re-run** — unrelated to Feature 15, not yet exercised.
- **[Carried, unresolved] LVD/end-of-discharge-voltage coordination (sizing spec Part E2) is not implemented anywhere in the pipeline.**
- **[Carried, unresolved] `validate_system_design`'s recharge-feasibility check is dead code.**
- **No `price` field on `BatteryCatalog`/`InverterCatalog`.**
- **Feature 09 CORS:** Production origin(s) resolved via `ALLOWED_ORIGINS` env var. Not blocking.
- **Numeric catalog fields beyond `lvd_threshold_v` still lack `allow_inf_nan=False`.**
- **[Carried] `LoadItem` field naming diverges from `battery_inverter_sizing_full_spec.md`.**
- **[Carried] Dashboard Filters drawer fields don't exist on the data model yet.**
- **[Carried] Pre-fix naive-UTC timestamps possibly present in older documents.**
...
11. **`LoadItem.power_factor` defaulted to `1.0` with no way to represent "unset," silently defeating category-based PF resolution** — fixed via `Optional[float] = None` in `models.py` plus explicit `_resolve_power_factor()` in `load.py`.
...
14. **`GET /api/projects/csv-template` returned 404, shadowed by `GET /{project_id}`** — FastAPI matches routes in registration order; the fixed-path route was declared after the parameterized one. Fixed by moving `download_csv_template_endpoint`'s registration above `GET /{project_id}`. Caught via the Feature 15 smoke test (Test 1a/1b/1c), not shipped silently.
15. **First Feature 15 smoke test run failed with `RuntimeError: Database has not been initialized`** — `TestClient(app)` was instantiated as a bare object instead of a context manager, so FastAPI's `lifespan` (which runs `connect_to_mongo()`) never fired. Fixed by wrapping all test calls in `with TestClient(app) as client:`. This was a smoke-test-script bug, not an application bug.
...
- **PF resolution policy:** `power_factor` on `LoadItem` is `Optional` — `None` means "resolve from `LOAD_CHARACTERISTICS[category]`," resolved in exactly one place (`load.py`'s `_resolve_power_factor()`).
- **Route registration order (Feature 15):** any fixed-path route (e.g. `/csv-template`) sharing a router with a `/{param}` route must be registered *before* the parameterized route — a FastAPI/Starlette matching-order rule, checked whenever a new literal-path endpoint is added.
- **CSV upload commit strategy (Feature 15):** strict all-or-nothing per-file validation, no partial commits. Rejection responses always carry a `template_url`. Append-only, reuses `repository.replace_project` rather than a new dedicated write path.
...
- Post-verification, the user raised the `datetime.utcnow()` deprecation warning visible in the smoke test output. Resolved as a small, immediate fix (not deferred to a future unit, since it touched already-shipped Feature 12/14 files directly) — `_utc_now()` helper added locally to both `models.py` and `repository.py`. This also prompted an explicit timestamp-storage-policy decision (UTC storage, IST/locale display deferred to frontend) rather than silently guessing which the user wanted — the user's IST request was clarified as a frontend display concern, not a backend storage change, and the reasoning was confirmed against `project-overview.md`'s existing multi-region scope before proceeding. The fix initially appeared incomplete because the smoke test script itself had a separate, unrelated `datetime.utcnow()` call in its own test-seeding code — this was fixed in the throwaway script as well, and the final verification run confirmed both the warning gone and all functional assertions still passing.
- **Feature 15 session summary:** spec finalized first (append-only, all-or-nothing commit, template pointer on rejection — all resolved against industry precedent and explicit user decisions). Code written against confirmed Feature 13 function signatures from the spec doc, with two assumptions flagged rather than guessed as certain; both confirmed correct once the user pasted real `backend/csv/` sources. Mid-session, an unrelated pre-existing bug surfaced and was fixed first (unresolved Git merge-conflict markers in `models.py`, then the `power_factor` default gap it was masking). A dedicated smoke test script was then written, went through two rounds of debugging — first a script-level `TestClient` lifespan bug (fixed), then a real route-ordering bug in `router.py` caught by the test itself (fixed) — before reaching a clean 23/23 pass. Per the user's standing workflow rules, the smoke test script itself is now marked for deletion, and this file has been updated to reflect Feature 15 as fully complete before moving to Feature 16.